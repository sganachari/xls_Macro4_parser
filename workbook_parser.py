    ...
    while hex_str!="":
        ...
        if  ptg_ordinal in Ptg.keys():
            ...
            if Ptg[ptg_ordinal]=='PtgExp':
                ...
                output.append("<Floating_numer_requires_impl>")
                hex_str = hex_str[9:]
...
def workbook(ole):
    ...
    with ole.openstream('Workbook') as fh:
        ...
        while fh.tell() < ole.get_size('Workbook'):
            ...
            if record_id in XLS_Record.values():
                if (XLS_Record.keys()[XLS_Record.values().index(record_id)] == 'XF'):
                    ...
                    output = ""
                    output += parse_ptg_records(cell_parsed_formula)
                    print ("Cell :: %s%d :: %s"%(int2xlscolumn(column+1),row+1,output))
                    match = None
                    match = re.match("SET.VALUE\(([A-Z]+\d+),(.*)\)$",output)
                    if match:
    ...
    d = decyrpt_values(set_values,other_forumla)
    print '*'*100
    print d
    print formulas_used
    return
